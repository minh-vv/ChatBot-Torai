...
from Cache.elasticache_standard import ElastiCacheMemory
import dify
...
def process_document(file_path, tool_name):
...
async def create_upload_file(file: UploadFile = File(...), tool_name: str = Form("default"), background_tasks: BackgroundTasks = None):
    """
...
@app.post("/dify/chat")
@app.post("/dify/chat/")
async def dify_chat_endpoint(request: DifyChatRequest, raw_request: FastAPIRequest):
    ...
    def event_generator():
        for chunk in dify.send_chat_message(request.user_id, request.conversation_id, request.query, request.files):
